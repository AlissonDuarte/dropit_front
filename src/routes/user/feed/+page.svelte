<script>
  ...
  async function fetchNotificatinoCount(){
    try {
      ...
      if (!response.ok) {
        triggerAlert(
          ...
          'Falha inexperada ao buscar notificações.'
        );
      }
...
<div class="flex min-h-screen bg-gradient-to-b from-zinc-900 to-zinc-800 text-zinc-100">
  ...
  <main class="flex-1 px-4 py-12 overflow-auto">
    ...
    <Feed {posts} cssClass="space-y-6 max-w-3xl mx-auto cursor-pointer" />
